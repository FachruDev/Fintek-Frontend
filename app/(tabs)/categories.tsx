  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity style={styles.newBtn} onPress={() => router.push('/category/new')}>
          <Text style={{ color: '#0B1110', fontWeight: '700' }}>+ New</Text>
        </TouchableOpacity>
      </View>

      <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={fetchData} tintColor="#22c55e" />}>
        <SectionHeader title="Income" />
        <View style={{ gap: 12 }}>
          {income.map((c) => (
            <CategoryRow key={c.id} item={c} onEdit={() => router.push(`/category/${c.id}`)} onDelete={() => requestDelete(c.id)} />
          ))}
          {income.length === 0 && <Text style={styles.empty}>No income categories.</Text>}
        </View>
        ...
        <View style={{ gap: 12, marginBottom: 40 }}>
          ...
          {expense.length === 0 && <Text style={styles.empty}>No expense categories.</Text>}
        </View>
      </ScrollView>
      <ConfirmModal
        visible={!!confirmId}
...
const styles = StyleSheet.create({
  ...
  newBtn: { backgroundColor: '#22c55e', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 999 },
